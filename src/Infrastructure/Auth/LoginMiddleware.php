...
final readonly class LoginMiddleware extends AbstractRedirectMiddleware
{
    ...
    {
        if ($request->getUri()->getPath() !== $this->appConfig->loginPath) {
        ...
        try {
            return $this->getRedirectResponse(
                $this->auth->login(
                    $this->getLoginRedirectUri($request)
                )
            );
    ...
    private function getLoginRedirectUri(RequestInterface $request): string
    {
        return join(
            "",
            [
