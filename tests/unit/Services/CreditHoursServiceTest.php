...
#[CoversClass(CreditHoursService::class)]
#[UsesClass(CreditHours::class)]
final class CreditHoursServiceTest extends TestCase
{
    private StorageContext&MockObject $storageContext;
    private CreditHoursService $creditHoursService;

    protected function setUp(): void
    {
        $this->storageContext = $this->createMock(StorageContext::class);
        $this->creditHoursService = new CreditHoursService($this->storageContext);
    }
    ...
    {
        $expectedEntry = new CreditHours(["id" => $id]);
        ...
        $result = $this->creditHoursService->get($id);

        $this->assertInstanceOf(CreditHours::class, $result);
    ...
    public function testGetReturnsNotFoundWhenEntryDoesNotExist(string $entryId): void
    {
        ...
        $result = $this->creditHoursService->get($entryId);
    ...
    public function ewioqpj(
        ...
        $this->creditHoursService->save($entry, $tenantId);
    }
    ...
    public function vmcxz(
        ...
        $entry = new CreditHours(["year" => 2025, "month" => 12]);
